...
class GetPaidCsvExportFormatter {
  ...
  List<String> _fields(GetPaidTransaction tx) {
    ...
    return [
      ...
      btcLegValueEst,
      creditedMinor != null ? _major(creditedMinor) : '',
      firstFiatLeg?.currency ?? '',
      firstFiatLeg?.executionRateMinorPerBtc != null
          ? _major(firstFiatLeg!.executionRateMinorPerBtc!)
          : '',
  ...
  // Fiat minor units → major.minor, dot decimal, unlocalized. Integer
  // arithmetic (never floating point); the seven supported currencies are all
  // 2-decimal.
  String _major(int minor) {
