...
class InvoicesPayServiceDatasource implements InvoicesPayServicePort {
      ...
      return Ok(
        CreateInvoiceResult(
          ...
          privateLink: PrivateInvoiceLink.fromServer(
            ...
            expectedNym: operation.linkToPageNym,
          ),
        ),
      );
    } on ArgumentError {
  ...
  InvoiceFallbackSupervision _toFallbackSupervision(
    ...
    return InvoiceFallbackSupervision(
      ...
      invoiceContext: InvoiceFallbackInvoiceContext(
        status: _invoiceStatus(item.invoice.status, operation: 'fallback'),
        amountSat: item.invoice.amountSat,
        fiatAmountMinor: item.invoice.fiatAmountMinor,
        fiatCurrency: item.invoice.fiatCurrency,
        memo: item.invoice.publicDescription ?? item.invoice.invoiceNumber,
        createdAt: _fromUnix(item.invoice.createdAtUnix),
      ),
    );
  }
