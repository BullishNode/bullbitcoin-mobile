...
class CreateInvoiceUsecase {
  ...
  Future<Result<CreateInvoiceResult, InvoicesFailure>> execute(
    ...
    return _operationLock.synchronized(() => _execute(command));
  }

  Future<Result<CreateInvoiceResult, InvoicesFailure>> _execute(
    CreateInvoiceCommand command,
  ) async {
  ...
  @useResult
  Future<Result<CreateInvoiceResult?, InvoicesFailure>> resumePending() {
    return _operationLock.synchronized(_resumePending);
  }

  Future<Result<CreateInvoiceResult?, InvoicesFailure>> _resumePending() async {
    final PreparedPrivateInvoiceCreate? pending;
    try {
      pending = await _links.getPending();
    } on Exception {
