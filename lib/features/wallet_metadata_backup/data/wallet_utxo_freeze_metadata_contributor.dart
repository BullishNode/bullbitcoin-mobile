...
final class WalletUtxoFreezeMetadataContributor
  ...
  final WalletUtxoRepository _walletUtxos;
  ...
  exportRecords() async {
    try {
      final freezes = await _walletUtxos.getAllFrozenWalletOutpoints();
  ...
  applyIntents({
    ...
    try {
      ...
      final existing = await _walletUtxos.getAllFrozenWalletOutpoints();
      final existingKeys = existing.map(_freezeKey).toSet();
      final alreadyPresentCount = desiredKeys
          .where(existingKeys.contains)
          .length;
      await _walletUtxos.restoreFrozenWalletOutpoints(desired);
      return Ok(
        WalletMetadataContributorApplySummary(
          ...
          localProjectionMatchesSnapshot: existingKeys.every(
