...
final class ApplyWalletMetadataRecoveryPlanUsecase {
  ...
  _execute({
    ...
    switch (rootResult) {
      ...
      case Ok(:final value):
        if (value.parentFingerprint !=
            plan.selectedHead.root.parentFingerprint) {
    ...
    final startedBlock = WalletMetadataBackupRecoveryBlock(
      ...
      snapshotRevision: selected.root.revision,
      eventCreatedAt: selected.rootEventCreatedAt,
      observedAt: now,
    );
    ...
    final status = switch ((plan.isOlderRestore, complete)) {
      (false, true) => WalletMetadataRecoveryApplyStatus.latestComplete,
      (false, false) => WalletMetadataRecoveryApplyStatus.latestIncomplete,
      (true, true) => WalletMetadataRecoveryApplyStatus.olderComplete,
      (true, false) => WalletMetadataRecoveryApplyStatus.olderIncomplete,
    };

    final finalStateResult = await _stateRepository.update((state) {
      if (status == WalletMetadataRecoveryApplyStatus.latestComplete) {
      ...
      final reason = plan.isOlderRestore
          ? WalletMetadataRecoveryBlockReason.olderSnapshot
          : WalletMetadataRecoveryBlockReason.incompleteApply;
      return state.recordRecoveryApplyBlocked(startedBlock.withReason(reason));
    });
    ...
    final effectiveStatus =
        ...
        ? WalletMetadataRecoveryApplyStatus.latestIncomplete
        : status;
  ...
  WalletMetadataBackupFailure? _preflight(WalletMetadataRecoveryPlan plan) {
    ...
    for (final section in plan.selectedHead.root.sections) {
