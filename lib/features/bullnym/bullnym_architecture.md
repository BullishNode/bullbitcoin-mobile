...
registration, deletion, lookup, recovery-address, donation-page, currency,
recipient-invoice, opaque wallet-backup transport, and Get Paid
transaction-history calls.
...
Bullnym exposes one authenticated opaque-object contract for the single
`wallet_backup` stream. Public keys are carried in signed request bodies rather
than URLs. The client signs the SHA-256 digest of the fixed NUL-separated
`bullbitcoin-wallet-backup-v1` message and never gives Bullnym an xprv.
...
encryption hides the sectioned wallet-backup envelope.
