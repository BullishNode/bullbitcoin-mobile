...
- **KC-6 posture.** Wallet 103 is hidden + autosweep-ON on create AND on recovery
  (the restore path re-applies it to every classified reservation).
- **Manifest-record-before-fundable + rollback.** `PreparePosWalletUsecase`
  ...
  the wallet back best-effort. Re-prepare is idempotent (`created:false`). The
  unified wallet-backup coordinator observes the committed manifest change; POS
  does not own or manually invoke backup lifecycle APIs.
...
The feature remains fail-closed unless pay2 advertises the exact
