...
typedef WalletBackupSignHash = FutureOr<String> Function(String hashHex);

final class WalletBackupSigner {
  final String publicKeyHex;
  final WalletBackupSignHash signHashHex;

  WalletBackupSigner({required String publicKeyHex, required this.signHashHex})
    : publicKeyHex = publicKeyHex.trim().toLowerCase() {
    if (!RegExp(r'^[0-9a-f]{64}$').hasMatch(this.publicKeyHex)) {
      throw ArgumentError.value(
        publicKeyHex,
        'publicKeyHex',
        'wallet backup public key must be 32-byte lowercase hex',
      );
    }
  }
}
