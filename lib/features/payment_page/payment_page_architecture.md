...
The existing signed `description` field is the Donation Page's short
description; there is no separate social-only wire field. The editor requires
1-120 user-perceived Unicode characters and applies the server's 512-byte
safety ceiling. It explains that the text appears on the Page and when its link
is shared. Bullnym owns image rendering and always adds the fixed Bull Bitcoin
logo; mobile neither uploads nor signs image bytes.

Archived legacy Pages remain editable before re-publication so descriptions
that predate the 120-character contract can be shortened instead of stranding
the Page in an unrecoverable archived state.
