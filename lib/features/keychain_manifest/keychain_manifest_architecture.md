...
The wallet-metadata backup is the second real encrypted-Nostr caller, so signed-event serialization/verification, bare authenticated ciphertext, bounded WebSocket mechanics, and per-relay transport outcomes now live in `core/nostr`. The shared transport receives an already serialized request and therefore owns no manifest filter, key role, tag, recency, consent, or recovery policy.

The manifest-specific relay repository still owns the exact author/kind/`d=manifest` request, caller-supplied `wss` relay aggregation, acceptance policy, authenticity mapping, deduplication, and newest-first ordering. Publish success requires at least one relay `OK true` for the exact event id. Fetch accepts only authentic wallet-manifest events for the derived manifest author, kind `30078`, and `d=manifest`. Individual relay failures and malformed relay events are collapsed into sanitized manifest outcomes, and raw WebSocket errors do not cross the feature boundary. Local wallet creation and manifest recording do not depend on relay publish or fetch success.
