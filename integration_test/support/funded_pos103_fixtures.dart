...
/// ISOLATION: a POS sale settles to the dedicated POS wallet 103 and NEVER to
/// the shared Lightning Address wallet 101. Funds arrive by REAL Bullnym
/// settlement — the coordinator pays the server-hosted POS checkout, and Bullnym
/// delivers L-BTC to a wallet-103 address it derives from the registered
/// descriptor — not by the payer sending directly to an app-derived address.
///
...
// Scenario defaults. The POS wallet 103 is a Liquid wallet; the chosen receive
// rail is the POS terminal's Lightning path (Boltz REVERSE swap LN->L-BTC,
// claimed into a wallet-103 descriptor address). A reverse (submarine) swap is
// NOT a Boltz CHAIN swap, so the 25,000-sat chain-swap minimum does NOT apply —
// only the server's LNURL min_sendable bounds it from below. 2,000 sats is the
// minimum practical value for the three-Liquid-hop journey (settlement onto 103,
// autosweep drain 103 -> default, return default -> coordinator), matching the
// Page-102 witness. At f6eec5127 each app-driven hop pays 0.1 sat/vByte against
// a 100-sat autosweep dust floor (RunAutoSweepUsecase._dustThresholdSat), so the
// final-hop output stays ~17x the dust floor.
...
/// lane guard value differs. NOTE: unlike the earlier design, NO mnemonic is
/// injected — the app creates and owns its own seed.
class FundedPos103Fixtures {
  ...
  /// Spec -> coordinator: the POS receive surface (terminal URL) and the run
  /// parameters. Written once the POS terminal and wallet 103 exist, before the
  /// spec starts waiting for the payment. It also publishes the shared Lightning
  /// Address wallet-101 receive address so the coordinator's independent oracle
  /// can prove the payment never touched it (destination isolation). The pay
  /// TARGET is the terminal URL (the server mints the invoice and derives the
  /// wallet-103 settlement address), not a pre-known 103 address.
  ...
  /// Spec -> coordinator: the final observed state (the discovered wallet-103
  /// receipt outpoint, autosweep + return txids, and closing balances incl. the
  /// isolated wallet-101 balance). Written last, atomically.
