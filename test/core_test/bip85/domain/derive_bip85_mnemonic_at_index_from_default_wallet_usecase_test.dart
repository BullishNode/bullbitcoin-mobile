...
void main() {
  ...
  test('derives and stores index when no derivation exists', () async {
    when(() => bip85Repository.fetch(derivation)).thenAnswer((_) async => null);
    when(
  ...
  test('reuses compatible existing derivation without storing again', () async {
    const fingerprint = 'fingerprint';
    when(
      () => bip85Repository.fingerprintFromXprv(xprv),
    ).thenReturn(fingerprint);
    when(() => bip85Repository.fetch(derivation)).thenAnswer(
      (_) async => _derivation(alias: alias, xprvFingerprint: fingerprint),
    );
  ...
  test('rejects existing derivation with different alias', () async {
    const fingerprint = 'fingerprint';
    when(
      () => bip85Repository.fingerprintFromXprv(xprv),
    ).thenReturn(fingerprint);
    when(() => bip85Repository.fetch(derivation)).thenAnswer(
      (_) async => _derivation(alias: 'Other', xprvFingerprint: fingerprint),
    );

    expect(
      () => usecase.execute(index: 77, alias: alias),
      throwsA(isA<Bip85DerivationConflictException>()),
    );
  });
