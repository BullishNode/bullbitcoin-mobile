...
void main() {
  ...
  test(
    'DefaultWalletXprvAdapter uses canonical xprv in either environment',
    () async {
      ...
      for (final (environment, network) in [
        ...
        expect(walletRepository.environment, environment);
        expect(seedRepository.fingerprints.single, seed.masterFingerprint);
        expect(xprv, startsWith('xprv'));
      }
    },
  );

  test('DefaultWalletXprvAdapter rejects mismatched seed material', () async {
    final seed = _zeroMnemonicSeed();
    final adapter = DefaultWalletXprvAdapter(
      ...
      seedRepository: _FakeSeedRepository(seed),
    );

    await expectLater(
      adapter.deriveDefaultWalletXprv(),
      throwsA(
        isA<LightningAddressException>().having(
          (error) => error.code,
          'code',
          'DefaultWalletSeedMismatch',
        ),
      ),
    );
  });
