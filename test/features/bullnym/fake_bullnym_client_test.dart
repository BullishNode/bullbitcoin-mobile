...
// T-POS-FAKE (pr26): the in-memory fake's POS behaviours - the substrate the
// §9/F10 pos matrix rows are exercised against. Kept at L0 because the fake is a
// pure Dart in-memory model with no binding dependency.
...
void main() {
  ...
  test('posMode faults only affect kind=pos, never the page', () async {
    ...
    expect(
      () => client.getDonationPage(nym: 'alice', kind: 'pos'),
      throwsA(isA<BullnymException>().having((e) => e.retryable, 'retryable', true)),
    );
    expect(
      (await client.getDonationPage(nym: 'alice', kind: 'payment_page')).header,
      'Page',
