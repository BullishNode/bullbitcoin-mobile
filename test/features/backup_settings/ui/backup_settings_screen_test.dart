...
void main() {
  ...
  testWidgets('requires explicit Bullnym storage disclosure before enabling', (
    ...
    await tester.tap(_metadataSwitch());
  ...
  testWidgets('deletes the remote copy only after confirmation', (
    ...
    final deleteButton = find.text('Delete remote backup');
  ...
  testWidgets('manual action is available for an enabled unblocked backup', (
    ...
    final backupNowButton = find.text('Back up now').last;
...
Future<void> _pump(WidgetTester tester, _StubBackupSettingsCubit cubit) async {
  ...
  final walletBackup = _FakeWalletBackupFacade();
...
Finder _metadataSwitch() {
  return find.ancestor(
    of: find.text('Wallet metadata backup'),
    matching: find.byType(SwitchListTile),
  );
}

final class _FakeWalletBackupFacade implements WalletBackupFacade {
  WalletBackupState get _state => WalletBackupState(
    enabled: false,
    dirty: false,
    dirtyRevision: 0,
