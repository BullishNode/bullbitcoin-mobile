...
void main() {
  ...
  testWidgets('requires explicit relay disclosure before enabling', (
    tester,
  ) async {
    ...
    expect(find.textContaining('public Nostr relays'), findsOneWidget);
    expect(
      find.textContaining('different Nostr identity and encryption key'),
      findsOneWidget,
    );
    expect(find.textContaining('does not delete data'), findsOneWidget);
  ...
  testWidgets(
    'acknowledged consent enables without silently re-acknowledging',
    (tester) async {
      final cubit = _StubBackupSettingsCubit(
        BackupSettingsState(metadataDisclosureAcknowledged: true),
      );
      await _pump(tester, cubit);
      ...
      expect(find.text('Back up private wallet metadata?'), findsNothing);
      expect(cubit.enabledCalls, [(enabled: true, disclosureAccepted: false)]);
    },
  );
  ...
  testWidgets('disabling does not require a disclosure decision', (
    ...
    final cubit = _StubBackupSettingsCubit(
      BackupSettingsState(
        ...
        metadataDisclosureAcknowledged: true,
      ),
    );
    await _pump(tester, cubit);
