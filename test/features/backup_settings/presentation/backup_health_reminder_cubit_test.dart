...
void main() {
  ...
  test(
    'evaluation waits for acknowledgement and then applies the newest input',
    () async {
      final pending = Completer<Result<void, BackupSettingsFailure>>();
      var evaluation = 0;
      when(
        () =>
            evaluate.execute(wallets: any(named: 'wallets'), arkBalanceSat: 0),
        ...
        evaluation++;
        return evaluation == 1 ? const Ok(decision) : const Ok(null);
      });
      when(
        () => acknowledge.execute(decision),
      ...
      await cubit.evaluate(wallets: [wallet], arkBalanceSat: 0);
      final saving = cubit.acknowledge();
