...
void main() {
  ...
  testWidgets('shows the empty state when no user key exists', (tester) async {
    ...
    expect(
      find.text('No Nostr keys have been created yet.'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('reveals subdued system rows only after the warning is accepted',
      (tester) async {
    await _pump(
      tester,
      FakeKeychainManifestFacade(
        keys: [userKeyRecord(), systemKeyRecord()],
      ),
    );
  ...
  testWidgets('names system keys from their role, not their stored purpose', (
    ...
    expect(
      find.text('Nostr NIP-05 Public Nym Verification'),
      findsOneWidget,
    );
    expect(find.text('whatever was stored'), findsNothing);
  });

  testWidgets('the create action opens the create form', (tester) async {
    await _pump(tester, FakeKeychainManifestFacade(keys: [userKeyRecord()]));

    await tester.tap(find.byIcon(Icons.add));
    await tester.pumpAndSettle();

    expect(find.text('Create Nostr key'), findsWidgets);
