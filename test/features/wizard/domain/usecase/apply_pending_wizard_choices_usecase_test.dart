...
void main() {
  ...
  setUp(() {
    ...
    when(() => metadataBackup.acknowledgeRelayDisclosure()).thenAnswer(
      (_) async =>
          Ok(WalletMetadataBackupState.initial.acknowledgeRelayDisclosure()),
    );
    when(
  ...
  test('flushes every touched field', () async {
    ...
    verify(() => metadataBackup.acknowledgeRelayDisclosure()).called(1);
  ...
  test(
    'disabling metadata backup does not acknowledge relay publication',
    () async {
      when(() => wizard.readPending()).thenAnswer(
        (_) async => const WizardChoices(
