...
void main() {
  ...
  group('createInvoice', () {
    ...
    test(
      'rejects a response route that does not match the requested nym',
      () async {
        for (final mismatch in <({String? requestedNym, String responseUrl})>[
          (requestedNym: null, responseUrl: 'https://bullpay.ca/alice/i/inv-1'),
          (
            requestedNym: 'alice',
            responseUrl: 'https://bullpay.ca/invoice/inv-1',
          ),
          (
            requestedNym: 'alice',
            responseUrl: 'https://bullpay.ca/bob/i/inv-1',
          ),
        ]) {
          reset(bullnym);
          when(
            ...
            (_) async => BullnymCreateInvoiceResponse(
              invoiceId: 'inv-1',
              invoiceUrl: mismatch.responseUrl,
            ),
          );
          ...
          final failure = _unwrapFailure(
            await datasource.createInvoice(
              signer: signer,
              operation: createOperation(linkToPageNym: mismatch.requestedNym),
            ),
          );
