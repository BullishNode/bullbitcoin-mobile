...
void main() {
  ...
  test(
    'keeps an incident visible when its invoice is outside the invoice page',
    () async {
      when(
        () => payService.listInvoices(
          signer: any(named: 'signer'),
      ...
      when(
        ...
        (_) async => Ok(
          InvoiceFallbackOverview(
            items: [
              InvoiceFallbackSupervision(
                invoiceId: InvoiceId('older-invoice'),
                nym: 'merchant',
                state: InvoiceFallbackState.confirming,
                payerAmountSat: 105000,
                invoiceSwapAmountSat: 100000,
      ...
      expect(result.invoices.single.id.value, 'older-invoice');
      expect(result.invoices.single.memo, 'older invoice');
      expect(
        result.invoices.single.fallbackState,
        InvoiceFallbackState.confirming,
      );
