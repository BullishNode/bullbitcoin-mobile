...
void main() {
  ...
  test(
    'overlapping creates retry one durable operation without replacing its key',
    () async {
      final firstSaveStarted = Completer<void>();
      final releaseFirstSave = Completer<void>();
      var saveAttempts = 0;
      final sent = <PreparedPrivateInvoiceCreate>[];
      links.onSavePendingAsync = (_) async {
        saveAttempts++;
        if (saveAttempts == 1) {
      ...
      when(
        ...
        return sent.length == 1
            ? const Err(InvoicesFailure.network())
            : Ok(result);
      });

      final first = usecase.execute(_command());
      await firstSaveStarted.future;
      final second = usecase.execute(_command());
      ...
      expect(
        _unwrapFailure(await first).kind,
        InvoicesFailureKind.outcomeUnknown,
      );
      expect(_unwrap(await second), result);
      expect(sent, hasLength(2));
      expect(
        sent.map((operation) => operation.encrypted.clientRequestId).toSet(),
        hasLength(1),
      );
      expect(cipher.encryptCalls, 1);
      expect(links.saveCalls, 1);
      expect(links.pending, isNull);
      expect(links.retained[invoiceId.value], result.privateLink);
    },
  );
...
class _FakeLinks implements PrivateInvoiceLinkRepository {
  ...
  Future<void> Function(PreparedPrivateInvoiceCreate)? onSavePendingAsync;
  ...
  Future<void> savePending(PreparedPrivateInvoiceCreate operation) async {
    ...
    await onSavePendingAsync?.call(operation);
    pending = operation;
  }
}
